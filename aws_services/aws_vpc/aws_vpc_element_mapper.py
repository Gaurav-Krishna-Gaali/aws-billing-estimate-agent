"""
Amazon VPC Element Mapper
Discovers and maps ALL interactive elements on the Amazon Virtual Private Cloud (VPC) configuration page
"""
...
def map_amazon_vpc_elements():
    ...
    with sync_playwright() as p:
        ...
        configurator = AmazonVPCConfigurator(page)
        
        # Navigate to Amazon VPC config
        if configurator.navigate_to_amazon_vpc_config():
            print("[INFO] Successfully navigated to Amazon VPC configuration page")
...
def print_detailed_summary(elements):
    ...
    print(f"\n{'='*80}")
    print("COMPLETE AMAZON VPC CONFIGURATION PAGE ELEMENT MAP")
    print(f"{'='*80}")
...
def analyze_amazon_vpc_capabilities(elements):
    ...
    print(f"\n{'='*80}")
    print("AMAZON VPC CONFIGURATION CAPABILITY ANALYSIS")
    print(f"{'='*80}")
    ...
    for button_id, details in buttons.items():
        ...
        if any(keyword in text.lower() for keyword in ['save', 'add', 'configure', 'select', 'choose', 'create', 'delete', 'vpc', 'network', 'subnet', 'gateway', 'route', 'security', 'nacl']):
