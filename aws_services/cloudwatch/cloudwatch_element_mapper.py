"""
CloudWatch Element Mapper
Discovers and maps ALL interactive elements on the CloudWatch configuration page
"""
...
class CloudWatchConfigurator(BaseAWSConfigurator):
    """CloudWatch configuration class"""
    ...
    def navigate_to_cloudwatch_config(self) -> bool:
        ...
        try:
            ...
            # Search for CloudWatch (try different search terms)
            search_terms = ["CloudWatch", "Amazon CloudWatch", "AWS CloudWatch"]
            for term in search_terms:
                ...
                print("[ERROR] Could not find CloudWatch service with any search term")
                return False
...
def map_cloudwatch_elements():
    """Map all CloudWatch configuration elements"""
    print("[INFO] Starting CloudWatch Element Mapping...")
    
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)
        page = browser.new_page()
        
        configurator = CloudWatchConfigurator(page)
        
        # Navigate to CloudWatch config
        if configurator.navigate_to_cloudwatch_config():
            print("[INFO] Successfully navigated to CloudWatch configuration page")
            ...
            # Save element map
            configurator.save_element_map("cloudwatch_complete_elements_map.json")
            
            # Take screenshot for reference
            configurator.take_screenshot("cloudwatch_config_page.png")
            
            print("\n[SUCCESS] CloudWatch element mapping completed!")
            print("[INFO] Files created:")
            print("  - cloudwatch_complete_elements_map.json (complete element mapping)")
            print("  - cloudwatch_config_page.png (screenshot for reference)")
            
        else:
            print("[ERROR] Failed to navigate to CloudWatch configuration page")
...
def print_detailed_summary(elements):
    ...
    print(f"\n{'='*80}")
    print("COMPLETE CLOUDWATCH CONFIGURATION PAGE ELEMENT MAP")
    print(f"{'='*80}")
...
def analyze_cloudwatch_capabilities(elements):
    """Analyze what CloudWatch configuration capabilities we have"""
    print(f"\n{'='*80}")
    print("CLOUDWATCH CONFIGURATION CAPABILITY ANALYSIS")
    print(f"{'='*80}")
    ...
    for button_id, details in buttons.items():
        ...
        if any(keyword in text.lower() for keyword in ['save', 'add', 'configure', 'select', 'choose', 'create', 'delete', 'monitor', 'alert']):
...
def main():
    """Main function"""
    print("[INFO] CloudWatch Element Mapper - Discovering ALL CloudWatch Configuration Options")
    ...
    with sync_playwright() as p:
        ...
        if configurator.navigate_to_cloudwatch_config():
            ...
            print_detailed_summary(elements)
            analyze_cloudwatch_capabilities(elements)
