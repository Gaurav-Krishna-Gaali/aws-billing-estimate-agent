"""
EC2 Element Mapper
Discovers and maps ALL interactive elements on the EC2 configuration page
"""
...
def map_ec2_elements():
    ...
    with sync_playwright() as p:
        ...
        configurator = EC2Configurator(page)
        
        # Navigate to EC2 config
        if configurator.navigate_to_ec2_config():
            print("[INFO] Successfully navigated to EC2 configuration page")
...
def print_detailed_summary(elements):
    ...
    print(f"\n{'='*80}")
    print("COMPLETE EC2 CONFIGURATION PAGE ELEMENT MAP")
    print(f"{'='*80}")
...
def analyze_ec2_capabilities(elements):
    ...
    print(f"\n{'='*80}")
    print("EC2 CONFIGURATION CAPABILITY ANALYSIS")
    print(f"{'='*80}")
    ...
    for button_id, details in buttons.items():
        ...
        if any(keyword in text.lower() for keyword in ['save', 'add', 'configure', 'select', 'choose', 'create', 'delete', 'ec2', 'instance', 'compute', 'storage', 'network', 'security']):
