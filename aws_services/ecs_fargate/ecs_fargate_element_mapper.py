"""
ECS Fargate Element Mapper
Discovers and maps ALL interactive elements on the ECS Fargate configuration page
"""
...
class ECSFargateConfigurator(BaseAWSConfigurator):
    """ECS Fargate configuration class"""
    ...
    def navigate_to_ecs_fargate_config(self) -> bool:
        ...
        try:
            ...
            # Search for ECS Fargate (try different search terms)
            search_terms = ["AWS Fargate", "ECS Fargate", "Amazon ECS", "AWS ECS", "Fargate", "Elastic Container Service"]
            for term in search_terms:
                ...
                print("[ERROR] Could not find ECS Fargate service with any search term")
                return False
...
def map_ecs_fargate_elements():
    """Map all ECS Fargate configuration elements"""
    print("[INFO] Starting ECS Fargate Element Mapping...")
    
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)
        page = browser.new_page()
        
        configurator = ECSFargateConfigurator(page)
        
        # Navigate to ECS Fargate config
        if configurator.navigate_to_ecs_fargate_config():
            print("[INFO] Successfully navigated to ECS Fargate configuration page")
            ...
            # Save element map
            configurator.save_element_map("ecs_fargate_complete_elements_map.json")
            
            # Take screenshot for reference
            configurator.take_screenshot("ecs_fargate_config_page.png")
            
            print("\n[SUCCESS] ECS Fargate element mapping completed!")
            print("[INFO] Files created:")
            print("  - ecs_fargate_complete_elements_map.json (complete element mapping)")
            print("  - ecs_fargate_config_page.png (screenshot for reference)")
...
def print_detailed_summary(elements):
    ...
    print(f"\n{'='*80}")
    print("COMPLETE ECS FARGATE CONFIGURATION PAGE ELEMENT MAP")
    print(f"{'='*80}")
...
def analyze_ecs_fargate_capabilities(elements):
    """Analyze what ECS Fargate configuration capabilities we have"""
    print(f"\n{'='*80}")
    print("ECS FARGATE CONFIGURATION CAPABILITY ANALYSIS")
    print(f"{'='*80}")
    ...
    for button_id, details in buttons.items():
        ...
        if any(keyword in text.lower() for keyword in ['save', 'add', 'configure', 'select', 'choose', 'create', 'delete', 'ecs', 'fargate', 'container', 'task', 'service', 'cluster']):
...
def main():
    """Main function"""
    print("[INFO] ECS Fargate Element Mapper - Discovering ALL ECS Fargate Configuration Options")
    ...
    with sync_playwright() as p:
        ...
        if configurator.navigate_to_ecs_fargate_config():
            ...
            print_detailed_summary(elements)
            analyze_ecs_fargate_capabilities(elements)
