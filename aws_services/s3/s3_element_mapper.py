"""
S3 Element Mapper
Discovers and maps ALL interactive elements on the S3 configuration page
"""
...
def map_s3_elements():
    ...
    with sync_playwright() as p:
        ...
        if configurator.navigate_to_s3_config():
            print("[INFO] Successfully navigated to S3 configuration page")
...
def print_detailed_summary(elements):
    ...
    print(f"\n{'='*80}")
    print("COMPLETE S3 CONFIGURATION PAGE ELEMENT MAP")
    print(f"{'='*80}")
...
def analyze_s3_capabilities(elements):
    ...
    print(f"\n{'='*80}")
    print("S3 CONFIGURATION CAPABILITY ANALYSIS")
    print(f"{'='*80}")
    ...
    for button_id, details in buttons.items():
        ...
        if any(keyword in text.lower() for keyword in ['save', 'add', 'configure', 'select', 'choose']):
